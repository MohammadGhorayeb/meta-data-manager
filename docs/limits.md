...
| **10** | For **M4A** files, if the original recording was made at a different quality setting, a faint trace of that earlier setting stays in the sound. | **Measured, and cleared by the full re-encode.** A test that tries to read the original's quality setting from the sound alone gets it right **88% of the time** on an untouched file — and drops to **chance** after we re-record it. Files that started from the same recording come out **byte-for-byte identical**. What we still cannot say is whether a *deeper* analysis than ours would find something (limit #6). | ✅ **Solved, within what we can measure** |
...
- **M4A audio-space residual (E-M4A-AUDIO):** the source's own quality setting is recoverable from the sound at **0.88 on unscrubbed and copy-tier files, falling to chance (0.58) after F3**; same-source producers become byte-identical. Engine identity (ffmpeg AAC vs Apple AAC) is **not separable even unscrubbed** (0.67) — those two encoders are near-twins spectrally (99.5%-energy bandwidth 15075 vs 15427 Hz at the same bitrate), so this corpus cannot answer the engine question either way, and says so rather than counting a weak control as a pass (limits #10, #12).
