...
## Status
Phase 0 (harness + spine), **Phase 1 images complete** (JPEG F1/F2/F3, PNG F1/F2), and **Phase 2 audio complete** — MP3 (F1 + F3, cross-engine A2 evidence), FLAC (F1 + F2, lossless A2) and M4A (F1/F2/F3, the format MAT2 refuses). **Phase 3 documents is underway**: **PDF F1 and F2 are implemented** — our own walker, serializer, content tokenizer and content-stream canonicaliser, recursive into embedded images — and both the revision-history and producer-channel experiments are measured (see below). Fully implemented and measured, **315 tests passing**. Per-format Pareto matrices are generated under `tests/harness/results/`, and the scrubber-fingerprint guard passes. University implementation project; a working tool is the deliverable. A plain-language progress report is at `docs/p1_report.pdf`; a benchmark vs standard tools is at `docs/benchmark.md`.
...
| **PDF**  | pass / pass / n-t  | fail\*\* / fail\*\* / n-t |
...
src/scrub/
  ...
  formats/pdf/             PDF handler: structure walker + our own serializer +
                           content-stream canonicaliser (F2) +
