...
## Status
Phase 0 (harness + spine), **Phase 1 images complete** (JPEG F1/F2/F3, PNG F1/F2), and **Phase 2 audio complete** — MP3 (F1 + F3, cross-engine A2 evidence), FLAC (F1 + F2, lossless A2) and M4A (F1/F2/F3, the format MAT2 refuses). **Phase 3 documents is underway**: **PDF F1 is implemented** — our own walker, serializer and content tokenizer, recursive into embedded images — and the revision-history experiment is measured (see below). Fully implemented and measured, **292 tests passing**. Per-format Pareto matrices are generated under `tests/harness/results/`, and the scrubber-fingerprint guard passes. University implementation project; a working tool is the deliverable. A plain-language progress report is at `docs/p1_report.pdf`; a benchmark vs standard tools is at `docs/benchmark.md`.
...
\*\* **PDF also carries two producers** — the *serializer* that laid out the bytes and the *layout engine* that typeset the page — so its A2 cell names which one leaked instead of averaging them. Measured across five producers rendering one document (Chrome/Skia, LibreOffice, macOS Quartz, and two pikepdf synthetics that differ in both serializer and layout style): at `raw` all three channels identify the producer; after **F1 the serializer channel is closed outright** — header, binary comment, xref kind, object streams, trailer keys and indirect `/Length` no longer separate anyone — while the layout channel (operators, number precision, font subsets, **glyph geometry**) is untouched, because F1 never rewrites a content stream. So the cell fails on the layout engine alone, and that is exactly what F2 is scoped from. `n-t` = not tested: F2 and F3 don't exist yet, and the matrix says so rather than guessing.
