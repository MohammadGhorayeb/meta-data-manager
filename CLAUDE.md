...
- **M4A carries two producers** — the muxer (box order/brand/`free` slack) and the AAC encoder (coded audio) — so its A2 cells name which channel leaked instead of averaging them. F2 normalizes the muxer losslessly (stream copy); F3 collapses same-source producers to byte-identical output, leaving the primary-encoding trace (Sorell's residual, audio side) as an open residual because no audio-space classifier has been run for M4A yet. ISOBMFF box walking lives in `standards/isobmff.py` — Phase 4's MP4/HEIC reuse it. The trap it exists for: `stco`/`co64` hold ABSOLUTE offsets, so removing any box before `mdat` requires patching them or the file decodes to garbage while still parsing.
